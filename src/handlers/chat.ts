...
function buildDesktopBrowserContext(): string {
  return [
    ...
    "",
    "You must NEVER use `browser_navigate` to send the CURRENT tab to a different",
    "site for this — it REPLACES whatever the user had open (their VS Code, the",
    "app frontend, a doc they were reading) and they lose it. That is a",
    "destructive mistake that has annoyed the user; do not do it.",
    "",
    "Also: `browser_tabs` with action \"new\" does NOT work here (Electron cannot",
    "create tabs over CDP — it errors). `mcp__aiide__open_tab` is the ONLY way to",
    "open a new tab. Do not claim you opened a tab unless you used open_tab.",
    "",
    "Use `browser_navigate` ONLY when the user explicitly means the CURRENT tab —",
    "e.g. \"navigate this tab to X\", \"go back\", \"reload\" — or on a tab you just",
    "opened/selected yourself for a multi-step task.",
    "",
